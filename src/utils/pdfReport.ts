...
export const downloadPdfReport = async (
  ...
  const addPageHeader = (title: string = 'Risk Intelligence Report') => {
    ...
    const splitDisclaimer = doc.splitTextToSize('SIMULATED DEMO DATA: No physical prototype, live sensor network, trained OviZero model or field-validated prediction system has been deployed.', pageWidth - margin * 2);
  ...
  const metrics = [
    { title: 'Top Risk Zone', value: rankedZones.length > 0 ? sanitizePdfText(formatZoneName(rankedZones[0].id, rankedZones[0].name)) : 'None' },
    { title: 'Highest illustrative scenario index', value: topRiskScore },
  ...
  doc.setFontSize(12);
  doc.setTextColor(nearBlack);
  doc.text('Risk Distribution', margin, currentY);
  currentY += 8;
  ...
  doc.addPage();
  addPageHeader('Priority Zones');
  
  autoTable(doc, {
    startY: margin + 30,
    head: [['Rank', 'Zone', 'Illustrative scenario index', 'Status', 'Mock Growth', 'Egg Count', 'Temp', 'Humidity', 'Action Required']],
  ...
  const dhText = `Total: ${dh.total} | Low Battery: ${dh.lowBattery} | Strong Signal: ${dh.strongSignal} | Medium Signal: ${dh.mediumSignal} | Weak Signal: ${dh.weakSignal} | Maintenance: ${dh.maintenance} | Offline: ${dh.offline}`;
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(mutedGray);
  ...
  autoTable(doc, {
    startY: margin + 40,
    head: [['Node', 'Location', 'Illustrative scenario index', 'Battery', 'Solar', 'Signal', 'Last Update', 'Eggs', 'Match Score', 'Maintenance State']],
  ...
  doc.addPage();
  addPageHeader('Intervention Status');
  ...
  const ivText = `Total: ${iv.total} | Active: ${iv.active} | Awaiting verification: ${iv.awaitingVerification} | Effect verified: ${iv.verified} | Needs attention: ${(iv.noEffect + iv.escalated)}`;
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(mutedGray);
  ...
  doc.addPage();
  addPageHeader('Diagnostic and Intervention Activity Logs');
